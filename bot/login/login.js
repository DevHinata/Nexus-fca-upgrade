// set bash title
process.stdout.write("\x1b]2;Goat Bot V3 - Fca By Mahi68x\x1b\x5c");

// Keep-alive server removed - dashboard handles port binding
...
let subTitle = `GoatBot V3@${currentVersion}- A simple Bot chat messenger use personal account`;
const subTitleArray = [];
...
const author = ("Created by NTKhang, Enhance by Cid Kageno");
const srcUrl = ("Source code: https://github.com/ntkhang03/Goat-Bot-V2");
const fakeRelease = ("ALL VERSIONS NOT RELEASED HERE ARE FAKE");

// --- REMOVED BROKEN APP.LISTEN BLOCK HERE ---
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        if (global.GoatBot.config.serverUptime.enable == true && !global.GoatBot.config.dashBoard?.enable && !global.serverUptimeRunning) {
                                // NOTE: We already started a Keep-Alive server at the top, so we can skip or modify this part
                                // to avoid port conflicts if it tries to use the same port.
                                // However, if the bot uses a different internal logic for uptime, we leave it be
                                // but ensure it doesn't crash the main process.
                                try {
                                        ...
                                        // We use a different port (e.g. 3001) for this internal dashboard if PORT is taken by our Keep-Alive server
                                        const PORT = 3001; 
